...
<div class="max-w-3xl mx-auto py-10 px-4">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-xl font-semibold text-gray-800">Dr. {{ $doctor->name }}</h1>
        <a href="{{ route('patient.messages.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Messages</a>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 space-y-4 mb-4 min-h-64">
        ...
        @empty
            <p class="text-center text-gray-400 text-sm py-8">Commencez la conversation</p>
        @endforelse
    </div>

    {{-- Zone de saisie --}}
    <div class="bg-white rounded-2xl shadow-sm p-4">
        <form method="POST" action="{{ route('patient.messages.store', $conversation) }}" class="flex gap-3">
            @csrf
